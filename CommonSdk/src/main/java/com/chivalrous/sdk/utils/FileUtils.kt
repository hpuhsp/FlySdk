...
object FileUtils {
    ...
    private const val MEDIA_CACHE_DIR = "CompressCache"

    /**
     * 获取App所有缓存目录
     */
    fun getAppCacheDirs(context: Context): List<String> {
        return listOf(getCompressPicPath(context), getPicSavePath(context))
    }
     ...
     * @return
     */
    private fun getPicSavePath(context: Context): String {
     ...
     * 获取保存路径
     */
    fun getSavePath(context: Context, name: String): String {
        var appCacheDir: File? = null
        if (Environment.MEDIA_MOUNTED == Environment.getExternalStorageState()
            && hasExternalStoragePermission(context)
        ) {
