...
import com.chivalrous.sdk.widget.dialog.CustomEditDialog
...
class NumberLiftView @JvmOverloads constructor(
    ...
    private fun calculatingAction(adder: Boolean) {
        ...
        if (count < minLimitValue) {
            count = minLimitValue
            ToastUtils.showShort(context.getString(R.string.reach_min_limit_value))
            return
        }
        if (count > maxLimitValue) {
            count = maxLimitValue
            ToastUtils.showShort(context.getString(R.string.reach_max_limit_value))
            return
        }
        ...
        override fun filter(
            ...
            try {
                ...
                if (isInRange(min, max, input)) {
                    ...
                    ToastUtils.showShort(context?.getString(R.string.out_of_input_range))
                }
