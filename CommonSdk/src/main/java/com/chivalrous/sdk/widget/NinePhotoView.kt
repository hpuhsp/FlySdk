...
class NinePhotoView @JvmOverloads constructor(
    ...
    init {
        val typedArray =
            context.obtainStyledAttributes(attrs, R.styleable.NinePhotoView)
        maxSelectCount =
            typedArray.getInt(R.styleable.NinePhotoView_np_max_select, 9)
        gridColumn =
            typedArray.getInt(R.styleable.NinePhotoView_np_columns, 4)
        showPlus =
            typedArray.getBoolean(R.styleable.NinePhotoView_np_show_plus, false)
        edgeMargin = typedArray.getDimensionPixelSize(
            R.styleable.NinePhotoView_np_item_margin,
            FastUtils.dip2px(context, 6f)
        )
        whiteSpacing = typedArray.getDimensionPixelSize(
            R.styleable.NinePhotoView_np_white_spacing,
            FastUtils.dip2px(context, 8f)
        )
        edgePadding = typedArray.getDimensionPixelSize(
            R.styleable.NinePhotoView_np_edge_spacing,
            FastUtils.dip2px(context, 15f)
        )
        initRecyclerView()
    }
    ...
    private fun initRecyclerView() {
        // Item高度
        itemHeight =
            (FastUtils.getScreenWidth(context) - edgePadding * 2 - gridColumn * whiteSpacing) / gridColumn
